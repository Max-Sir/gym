    ...
    <div class="hello_message">
        <c:choose>
            <c:when test="${sessionScope.user == null}">
                ...
                <a class="logout_a" href="${pageContext.request.contextPath}/jsp/common/login.jsp">${pageScope.login}</a>
                <a class="register_login_a" href="${pageContext.request.contextPath}/jsp/common/register.jsp">${pageScope.register}</a>
            </c:when>
            <c:otherwise>
                <span class="hello_text">${pageScope.hello} ${sessionScope.user.firstName} ${sessionScope.user.lastName}</span>
                <a class="register_login_a" href="${pageContext.request.contextPath}/controller?command=common_logout">${pageScope.logout}</a>
            </c:otherwise>
        </c:choose>
    </div>
</header>
        ...
        <c:if test="${sessionScope.user != null}">
            ...
            <c:choose>
                <c:when test="${sessionScope.user.userRole == 'TRAINER'}">
                    <li><a href="${pageContext.request.contextPath}/controller?command=trainer_prepare_training_program_creation">${pageScope.create_training_program}</a></li>
                    <li><a href="${pageContext.request.contextPath}/jsp/trainer/create_exercise.jsp">${pageScope.create_exercise}</a></li>
                    <li><a href="${pageContext.request.contextPath}/controller?command=trainer_show_personal_clients">${pageScope.show_personal_clients}</a></li>
                </c:when>
                <c:when test="${sessionScope.user.userRole == 'ADMIN'}">
                    <li><a href="${pageContext.request.contextPath}/controller?command=admin_show_all_clients">${pageScope.show_clients}</a></li>
                    <li>
                ...
                <c:otherwise>
                    <li><a href="${pageContext.request.contextPath}/controller?command=special_show_client_orders&client_id=${sessionScope.user.id}">${pageScope.orders_history}</a></li>
                    <li><a href="${pageContext.request.contextPath}/controller?command=special_show_client_training_program&client_id=${sessionScope.user.id}">${pageScope.my_training_program}</a></li>
                    <li><a href="${pageContext.request.contextPath}/controller?command=client_check_actual_order&client_id=${sessionScope.user.id}">${pageScope.make_order}</a></li>
                </c:otherwise>
            </c:choose>
        </c:if>
    </ul>
