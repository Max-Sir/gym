...
<form id="createTrainingProgram" method="POST" action="${pageContext.request.contextPath}/controller">
    ...
    <p><label>${pageScope.select} <select name="client_id">
        <c:forEach var="clients" items="${sessionScope.list}">
            ...
            <option value="${clients.key}"><span>&#8470;</span> ${count} ID: ${clients.key} ${clients.value}</option>
        </c:forEach>
    </select></label></p>
    <p><label>${pageScope.start_date} <input title="${pageScope.title_start}" id="purchase_date" type="date" name="start_date" pattern="yyyy-MM-dd" onchange="checkStartDate()"/></label></p>
    <p><label>${pageScope.end_date} <input title="${pageScope.title_end}" id="end_date" type="date" name="end_date" pattern="yyyy-MM-dd" onchange="checkEndDate()"/></label></p>
    <p><label>${pageScope.days_count} <input title="${pageScope.title_count}" id="training_days" class="create_training_program_number" type="number" name="days_count" onchange="checkDaysCount()" value="2"/></label></p>
    <p>${pageScope.diet}</p>
    <p>
        <textarea title="${pageScope.title_diet}" id="diet_area" name="diet" onkeyup="checkDiet()"></textarea>
    </p>
    <button type="submit" id="button_create" disabled>${pageScope.create} <i class="fa fa-plus-circle" aria-hidden="true"></i></button>
</form>
</div>
<script>
    <jsp:directive.include file="/js/trainingProgramValidation.js"/>
</script>
</body>
</html>
